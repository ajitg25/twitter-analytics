...
from auth import init_auth_state, handle_oauth_callback, is_authenticated, get_current_user, logout

# Page config (Must be first)
st.set_page_config(
    page_title="Archive Analysis - Twitter Analytics", 
    page_icon="📁", 
    layout="wide",
...
st.markdown("""
    ...
    [data-testid="stFileUploader"] section[data-testid="stFileUploaderDeleteBtn"] { display: none; }
    [data-testid="stFileUploader"] section > button { display: none; }
    div[data-testid="stFileUploadDropzone"] { padding: 30px; }
    </style>
""", unsafe_allow_html=True)
...
if not uploaded_files:
    ...
    try:
        # Use root directory for demo data (parent of pages/__file__)
...
followers = data.get('followers', []) if data else []
following = data.get('following', []) if data else []
...
with col2:
    if st.button(f"👥 Fans (You don't follow back) ({len(followers_not_following_back)})", use_container_width=True):
